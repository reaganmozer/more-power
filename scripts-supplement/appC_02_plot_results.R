...
res = res.embed %>% group_by(model, n.coded) %>% arrange(.n,seed) %>% mutate(index=row_number())
...
ggsave("figures/supp-fig2.pdf", g2, width=6, height=4.5, units="in")
...
g3=ggplot(res3[res3$meth=="ML",],aes(x=n.coded,y=RE,col=group))+geom_line()+geom_point()+theme_bw()+
  ...
  theme(legend.position = c(0.75,0.75)) +guides(col=guide_legend(""))+cols+
  scale_x_continuous(limits=c(80,1000),breaks=seq(100,1000,by=100))
ggsave("figures/supp-fig3.pdf", g3, width=6, height=4.5, units="in")


g4=ggplot(res3[res3$meth=="ML",],aes(x=n.coded,y=PRB,col=group))+geom_line()+geom_point()+theme_bw()+
...
ggsave("figures/supp-fig4.pdf", g4, width=6, height=4.5, units="in")


g5=ggplot(res3[res3$meth%in%c("ML","synth"),],aes(x=n.coded,y=coverage,lty=meth,col=group))+geom_line()+geom_point()+
  scale_y_continuous(limits=c(0.1,1),breaks=c(seq(0.1,0.9,0.1),0.95,1),oob=scales::oob_squish) + theme_bw()+
...
ggsave("figures/supp-fig5.pdf", g5, width=6, height=4.5, units="in")
